...
class ScoringController extends Notifier<ScoringState?> {
  ...
  Future<void> addBall({
    ...
    if (state!.currentPhase == MatchPhase.completed || state!.currentPhase == MatchPhase.inningsBreak) return;
    ...
    try {
      ...
      double srikerSR = newBallsFaced > 0 ? (newBatsmanRuns / newBallsFaced) * 100 : 0.0;
      ...
      int newWickets = activeBowler.wickets + ((wicketType != null && wicketType != 'run_out') ? 1 : 0);
      int newDotBalls = activeBowler.dotBalls + (totalDeliveryRuns == 0 ? 1 : 0);
      double newEconomy = newOvers > 0 ? (newBowlerRuns / newBallsBowled * 6) : 0.0;
      ...
      if (newValidBalls == 6) {
        ...
        // Maiden Detection
        int runsInThisOver = 0; // In a full professional app, we'd sum currentOverBalls, but let's approximate or just flag. 
      ...
      if (newValidBalls == 0 && isValidBall) {
        ...
        if (runsInOver == 0) {
           bwStats[st.bowlerId] = bwStats[st.bowlerId]!.copyWith(maidens: bwStats[st.bowlerId]!.maidens + 1);
        }
      ...
      if (st.currentPhase == MatchPhase.firstInnings) {
        if (newWicketCount == 10 || (currentMatch.overs * 6) == (newCompletedOvers * 6 + newValidBalls)) {
            nextPhase = MatchPhase.inningsBreak;
        }
      } else if (st.currentPhase == MatchPhase.secondInnings && st.targetScore != null) {
        if (newTotalRuns >= st.targetScore!) {
           nextPhase = MatchPhase.completed;
           int ballsRem = (currentMatch.overs * 6) - (newCompletedOvers * 6 + newValidBalls);
           resultMsg = "${currentMatch.teamBName} won by ${10 - newWicketCount} wickets with $ballsRem balls remaining";
        } else if (newWicketCount == 10 || (currentMatch.overs * 6) == (newCompletedOvers * 6 + newValidBalls)) {
           nextPhase = MatchPhase.completed;
           if (newTotalRuns == st.targetScore! - 1) {
              resultMsg = "Match Tied";
              nextPhase = MatchPhase.superOver;
           } else {
              resultMsg = "${currentMatch.teamAName} won by ${st.targetScore! - newTotalRuns - 1} runs";
           }
        }
      }

      int? rNeeded = st.targetScore != null ? (st.targetScore! - newTotalRuns) : null;
      int? balRem = (currentMatch.overs * 6) - (newCompletedOvers * 6 + newValidBalls);
      double? rrr = (rNeeded != null && rNeeded > 0 && balRem > 0) ? (rNeeded / (balRem / 6)) : null;
      ...
      await ref.read(matchRepositoryProvider).saveDeliveryBatch(
        match: updatedMatch, 
  ...
  void updatePlayers(String newStriker, String newNonStriker, String newBowler, String strikerName, String nonStrikerName, String bowlerName) {
     if (state == null) return;
     
     final bStats = Map<String, BatsmanStats>.from(state!.batsmanStats);
     if (!bStats.containsKey(newStriker)) bStats[newStriker] = BatsmanStats.initial(newStriker, strikerName);
     if (!bStats.containsKey(newNonStriker)) bStats[newNonStriker] = BatsmanStats.initial(newNonStriker, nonStrikerName);
     
     final bwStats = Map<String, BowlerStats>.from(state!.bowlerStats);
     if (!bwStats.containsKey(newBowler)) bwStats[newBowler] = BowlerStats.initial(newBowler, bowlerName);
  ...
  Future<void> undo() async {
    ...
    try {
      ...
      await ref.read(matchRepositoryProvider).deleteBall(lastBall.matchId, lastBall.inningsId, lastBall.ballId);
...
final scoringControllerProvider = NotifierProvider<ScoringController, ScoringState?>(() {
